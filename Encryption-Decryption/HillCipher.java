...
public class HillCipher {
  ...
  public void divide(String temp, int s) {
    ...
    if (temp.length() == s) perform(temp); else if (temp.length() < s) {
      for (int i = temp.length(); i < s; i++) temp = temp + 'x';
      perform(temp);
    }
  }
  ...
  public boolean check(String key, int len) {
    ...
    if (d == 0) {
      System.out.println(
        "Invalid key!!! Key is not invertible because determinant=0..."
      );
      return false;
  ...
  public int determinant(int A[][], int N) {
    ...
    if (N == 1) res = A[0][0]; else if (N == 2) {
      ...
      for (int j1 = 0; j1 < N; j1++) {
        ...
        res +=
          Math.pow(-1.0, 1.0 + j1 + 1.0) * A[0][j1] * determinant(m, N - 1);
      }
    }
    return res;
  }
  ...
  public static void main(String args[]) throws IOException {
    ...
    if (sq != (long) sq) System.out.println(
